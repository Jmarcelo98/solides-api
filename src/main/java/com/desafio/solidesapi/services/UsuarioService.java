...
public class UsuarioService {
	...
	public Usuario buscarUsuarioLogado(String login) throws UsernameNotFoundException {
		return usuarioRepository.findByLoginIgnoreCase(login).orElseThrow(() -> new ResourceNotFoundException("Usuário não encontrado"));
	}
	...
	public UsuarioDTO criar(UsuarioRecord usuarioRecord) {
		...
		var usuario = Usuario.builder().id(null).login(usuarioRecord.login()).senha(bCrypt.encode(usuarioRecord.senha())).build();
